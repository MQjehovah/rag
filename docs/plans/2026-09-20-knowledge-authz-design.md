...
- **组内内容**：仅该组成员可见、可编辑；非成员访问一律 404（不泄露存在性）。
- **公共内容（`group_id IS NULL`）= 共享可编辑**：所有登录用户可见**且可写**。
  因此「删除/重命名公共笔记本（连带其笔记）」「删除/编辑公共笔记」「编辑公共 wiki 页」是**有意行为，不是漏洞**。任何登录用户都能修改公共内容，属于团队共享空间的既定模型。
- 管理员（`__local_admin__`）对全部内容可见、可写，并可设置归属组。
- 若后续需要「公共只读」，需显式改变本策略并单独实施，不应被当作缺陷顺手修改。
